...
6. **Updates.** "Update my exocortex" is all they need to say — the `update-exocortex` skill checks for releases, shows what changed, reconciles their amendments, and refreshes program files only, never their data. The weekly digest also surfaces new releases on its own; if they use GitHub, additionally suggest watching the program repo (**Watch → Custom → Releases**) for release notifications.
