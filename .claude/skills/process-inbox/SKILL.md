...
1. Walk `notes/` recursively. OS artifacts (`.DS_Store`, `Thumbs.db`, `desktop.ini`) and `notes/README.md` (folder law, not a note) are invisible to the sweep — never walked, ingested, or carried in the cursor.
...
8. Regenerate `notes/index.md` — the one agent-authored file in notes/: categorized sections of `* [[Note]] - description` lines from note frontmatter. Wikilinks, not paths — they survive the user moving notes; path-qualify (`[[notes/path/Name|Name]]`) only names that aren't unique vault-wide. (The wiki's markdown-link rule doesn't apply: notes/ never ships.) Preserve existing category assignments on regeneration — slot only new or changed notes; the user may recategorize freely and those choices stick. The index itself is invisible to the sweep — never walked, hashed, or ingested.
